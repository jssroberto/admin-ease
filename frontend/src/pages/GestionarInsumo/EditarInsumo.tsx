...
const EditarInsumo: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await axios.put(`http://localhost:8080/api/v1/insumo/${id}`, formData);
      navigate('/gestionar-insumos');
    } catch (err) {
      setError('Error updating insumo');
      console.error(err);
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="flex flex-col max-w-4xl mr-auto p-6">
      <h1 className="text-2xl font-medium mb-6">Editar Insumo</h1>
